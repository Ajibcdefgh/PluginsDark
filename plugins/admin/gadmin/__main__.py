...
@userge.on_cmd("zombies",
               about={'header': "gunakan ini untuk membersihkan akun zombie",
                      'description': "periksa & hapus akun zombie (dihapus) dari supergroup.\n"
                      "[CATATAN: Membutuhkan hak admin yang tepat dalam obrolan!!!]",
                      'flags': {'-c': "clean"},
                      'examples': ["{tr}zombies [periksa akun yang dihapus di grup]",
                                   "{tr}zombies -c [hapus akun yang dihapus dari grup]"]},
               allow_channels=False,
               allow_bots=False,
               allow_private=False)
async def zombie_clean(message: Message):
    ...
    if rm_delaccs:
        ...
        del_total = 0
        if can_clean:
            await message.edit("`Tunggu sebentar!! membersihkan akun zombie dari obrolan ini..`")
            async for member in message.client.get_chat_members(chat_id):
                if member.user.is_deleted:
                    ...
                    del_total += 1
            if del_admins > 0:
                del_stats = f"`👻 Ditemukan` **{del_total}** `total zombie..`\
                \n`🗑 Dibersihkan` **{del_users}** `akun zombie (dihapus) dari obrolan ini..`\
                \n🛡 **{del_admins}** `akun admin yang dihapus dilewati!!`"
            else:
                del_stats = f"`👻 Ditemukan` **{del_total}** `total zombie..`\
                \n`🗑 Dibersihkan` **{del_users}** `akun zombie (dihapus) dari obrolan ini..`"
            ...
            await CHANNEL.log(
                "#ZOMBIE_CLEAN\n\n"
                f"OBROLAN: `{message.chat.title}` (`{chat_id}`)\n"
                f"JUMLAH TOTAL ZOMBIE: `{del_total}`\n"
                f"JUMLAH ZOMBIE YANG DIBERSIHKAN: `{del_users}`\n"
                f"JUMLAH ADMIN ZOMBIE: `{del_admins}`")
        else:
            await message.err(r"saya tidak memiliki izin untuk melakukan itu! (* ￣︿￣)")
    else:
        del_users = 0
        del_stats = r"`Tidak ada akun zombie ditemukan di obrolan ini... WOOHOO grupnya bersih.. \^o^/`"
        await message.edit("`🔎 Mencari akun zombie di obrolan ini..`")
        async for member in message.client.get_chat_members(chat_id):
            if member.user.is_deleted:
                del_users += 1
        if del_users > 0:
            del_stats = f"`Ditemukan` **{del_users}** `akun zombie di obrolan ini.`"
            await message.edit(
                f"🕵️‍♂️ {del_stats} `Anda dapat membersihkannya menggunakan .zombies -c`", del_in=5)
            await CHANNEL.log(
                "#ZOMBIE_CHECK\n\n"
                f"OBROLAN: `{message.chat.title}` (`{chat_id}`)\n"
                f"JUMLAH ZOMBIE: `{del_users}`")
        else:
            await message.edit(f"{del_stats}", del_in=5)
            await CHANNEL.log(
                "#ZOMBIE_CHECK\n\n"
                f"OBROLAN: `{message.chat.title}` (`{chat_id}`)\n"
                r"JUMLAH ZOMBIE: `WOOHOO grupnya bersih.. \^o^/`")
