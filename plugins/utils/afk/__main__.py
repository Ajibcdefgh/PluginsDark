...
async def active_afk(message: Message) -> None:
    ...
    await asyncio.gather(
        CHANNEL.log(f"Anda pergi AFK! : `{REASON}`"),
        message.edit("`Anda pergi AFK!`", del_in=1),
        AFK_COLLECTION.drop(),
...
async def handle_afk_incomming(message: Message) -> None:
    ...
    if user_id in USERS:
        ...
        if REASON:
            out_str = (f"Aku **AFK** untuk saat ini.\nAlasan: <code>{REASON}</code>\n"
                       f"Terakhir terlihat: `{afk_time} yang lalu`")
        else:
            out_str = choice(AFK_REASONS)
        coro_list.append(message.reply(out_str))
...
async def handle_afk_outgoing(message: Message) -> None:
    ...
    IS_AFK = False
    afk_time = time_formatter(round(time.time() - TIME))
    replied: Message = await message.reply("`Aku tidak lagi AFK!`", log=__name__)
    coro_list = []
    if USERS:
        p_msg = ''
        g_msg = ''
        ...
        coro_list.append(replied.edit(
            f"`Anda telah menerima {p_count + g_count} pesan saat Anda pergi. "
            f"Check log for more details.`\n\n**waktu AFK** : __{afk_time}__", del_in=3))
        out_str = f"Anda telah menerima **{p_count + g_count}** pesan " + \
            f"dari pengguna **{len(USERS)}** saat Anda pergi!\n\n**waktu AFK** : __{afk_time}__\n"
        if p_count:
